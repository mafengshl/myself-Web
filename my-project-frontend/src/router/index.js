...
const router = createRouter({
    ...
    routes: [
        ...
        // 添加相册页面路由
        {
            path: '/gallery',
            name: 'Gallery',
            component: () => import('@/views/components/card/Gallery.vue')
        },
        // 添加简历页面路由
        {
            path: '/job',
...
router.beforeEach((to, from, next) => {
    ...
    const authPages = ['welcome-login', 'welcome-register', 'welcome-forget', 'login', 'register', 'forget']
    if(authPages.includes(to.name) && !isUnauthorized) {
        next('/index')
    }
    else {
        next()
